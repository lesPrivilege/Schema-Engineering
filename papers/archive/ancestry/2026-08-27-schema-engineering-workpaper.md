...
Status: WorkPaper
Edition: 2026-08-27
Scope: 从专家演示到 Agent Extension、从概率性提议到正式工作、从 Context / Output 治理到 Work Eval 与 Post-agentic Refinement 的完整方法、适用边界与产品纪律。
---

# Schema Engineering：让工作存在于模型之外

## 从专家演示到可验收 Agent Extension 的弱编译与工作编纂方法
...
这一方法同时治理 Run 的两端。输入侧把 Stable Contract、Current Semantic State、相关资源与可检索历史投影为 task-specific Context；输出侧把 Model / Human Proposal 视为 Candidate Change，只有经过验证、授权和适用的 Review 才能进入 Committed Work State。单次输出质量因此不等于跨时间工作的质量；只要 output 会成为后续工作的 input，运行之后“留下什么、失效什么、下一次优先看什么”就是产品能力的一部分。
...
法律提供了高要求实例，但不构成外延边界。财务、医疗、咨询、研究、工程、合规、审计等工作同样需要来源、权限、版本、审阅和责任。Schema Engineering 的高价值对象不是所有 LLM 任务，而是语言模型能够触达、跨时间持续、具有状态与责任、又缺乏廉价稳定外部 verifier 的专业工作。各领域可以共享 Runtime primitives，不能因此省略各自的专业语义。
...
> **Schema Engineering 是一种面向概率性执行者的弱编译与工作编纂方法：它把专家在真实工作中演示出来的隐性义务、状态、权限、证据、完成标准与裁决边界编订为可版本化、可执行、可验证、可恢复的 Work Contract，以同一套 Schema 治理 Matter 中 Sources、Events、State 与 Artifacts 的关系，并据此发布可通过端到端验收、能够在生产 Review 中继续改进的 Agent Extension。**
...
前者解决“专家亲自跑通的 Demo 如何成为其他人可用的能力”；后者解决“模型输出凭什么改变正式状态”。只写后者，Schema Engineering 会退化为 Professional Work Runtime；只写前者，它会退化为专家 workflow 产品化。两项转换共同发生在一条持续的 Context / State 循环中：正式状态被投影给一次执行，一次执行的结果再经过窄化提交改变正式状态。
...
#### 8.3 Model、Agentic 与 Work benchmark
...
#### 12.1 适用范围：longitudinal, stateful, unverifiable professional work

Schema Engineering 不是“所有 LLM 应用都需要额外 Schema”。它的价值随工作连续性、状态复杂度和验证缺口增加。一个场景越符合以下条件，显式 Work Contract 与状态治理越有价值：
...
Professional Work Runtime 是一种以长期工作事项和正式成果为中心的系统：模型在可执行契约下完成工作，系统在对话之外维护状态、证据、权限、完成条件、Context Projection 与可恢复历史，Accountable Principal 在需要最终问责的节点裁决。

这一定义形成一类产品范式，但还不能单凭概念完整性证明一个天然成立的横向市场品类。不同领域的 Artifact、Evidence、Authority、Completion、Review 与 Work benchmark 语义并不相同。共享 Runtime primitives 可以降低重复建设，不能替代垂直产品对专业工作的定义。
...
| Context / state governance | 是否降低重复注入、状态重判、漂移与恢复成本 |
...
Schema Engineering 的产品命题、学习命题与市场命题具有不同证据强度。Matter-first、external Semantic State、Evidence、Authority、Completion、Review、Operational Responsibility、Accountability、Context Projection、Candidate Output commitment 与 Lane parallelism 属于产品结构判断；Work benchmark 的增量解释力、attention economics、Production Learning、institution-level generalization、synthetic economics、refinement capability 与横向市场品类仍需实证。抗折旧判断也建立在“有限 attention 下由 Context 生成 Output，且工作经验不能可靠持续写回参数”的条件上，不是无条件的技术预言。
...
| F19 | 实际高价值使用主要是一次性任务，Output 很少进入后续工作，持久状态与 Context Projection 不改善留存、成果质量、恢复或成本 | P20 与 longitudinal product wedge 缩到少数连续性场景；其余产品退回一次性 assistant |

每项证伪条件都需要配套测试设计。长期连续性比较应包含中断、跨周工作与模型替换；Ontology 比较应覆盖责任结构不同的至少三个领域；Contract 比较应同时测开放问题发现、锚定、错误确信、成果接受率、token、latency 与维护成本；Model / Agentic / Work benchmark 比较应固定模型、Harness 与任务分层，检查对 accepted work product 和 reversal 的增量预测力；学习路线比较应固定 expert-hour、计算和数据预算；Harness 测试应在语义等价的接口变化下进行；产品边界应以真实采购、替换、留存和支付行为验证；增量采用测试应固定 Matter 范围，完整计入编码、集成、迁移和持续维护成本，再与 Review、恢复、Context 构造和版本协调收益比较。
...
Compilation、Ontology / Continuity 与 Commitment 三组共同定义 Schema Engineering Kernel；Evaluation Principles 规定 Kernel 如何从 Work Contract 派生测试并保持解释边界；Product Governance 和 Evolution Strategy 是从 Kernel 推导出的运行纪律，需要单独验证。P17 描述专业能力编译的发布门，P18 描述 Context / Output 的连续性边界，P19 描述 Work Eval 的语义来源，P20 描述适用范围与长期质量，不改写 Matter、Event、Semantic State、Artifact 与 Review 的 Runtime ontology。
